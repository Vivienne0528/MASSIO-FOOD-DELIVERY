...
import { ProductType } from "@/types/types";  // Your types for products
...
export default async function CategoryPage({ params }: { params: { category: string } }) {
  ...
  const products: ProductType[] = await fetchData(category);  // Fetch the products based on category

  return (
    <div className="flex flex-wrap text-red-500">
      {products.map((item) => (
        <div key={item.id} className="w-full h-[60vh] border-r-2 border-b-2 border-red-500 sm:w-1/2 lg:w-1/3 p-4 flex flex-col justify-between group odd:bg-fuchsia-50">
          {/* IMAGE CONTAINER */}
          {item.img && (
            <div className="relative h-[80%]">
              <Image src={item.img} alt="" fill className="object-contain" />
